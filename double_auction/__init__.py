...
class Constants(BaseConstants):
    ...
    INSTRUCTIONS_TEMPLATE = "double_auction/Instructions.html"
...
class Group(BaseGroup):
    ...
    def sequence_lengths(self):
        """
        Returns (s0, s1, s2, s3, s4) as ints from session.config.
        """
        s0 = self.cfg_int("sequence_0", 0)
        s1 = self.cfg_int("sequence_1", 0)
        s2 = self.cfg_int("sequence_2", 0)
        s3 = self.cfg_int("sequence_3", 0)
        s4 = self.cfg_int("sequence_4", 0)
        return s0, s1, s2, s3, s4

    def has_sequence_4(self) -> bool:
        # your original logic: sequence_4 != 0 toggles an extra block
        return self.sequence_lengths()[-1] != 0
    ...
    def get_spread_html(self, house):
        tpl = (
            "double_auction/includes/spread_to_render_btc.html"
            if house is True
            else "double_auction/includes/spread_to_render.html"
        )
    ...
    def final_round_in_sequence(self):
        s0, s1, s2, s3, s4 = self.sequence_lengths()
        if self.has_sequence_4():
            return [
                s0,
                s0 + s1,
                s0 + s1 + s2,
    ...
    def set_sequence(self):
        sequence_list = self.final_round_in_sequence()
        i = 0
        for j in range(len(sequence_list)):
            if self.round_number > sequence_list[j]:
                i += 1
        self.sequence_number = i
        if i > 0:
            self.round_within_sequence = self.round_number - sequence_list[i - 1]
        else:
            self.round_within_sequence = self.round_number

    def set_total_round(self):
        sequence_list = self.final_round_in_sequence()
        self.total_round = sequence_list[-1]
    ...
    def fake_draw(self):
        if self.round_number in self.final_round_in_sequence():
            self.drawn_number = 1
        else:
            self.drawn_number = random.randint(2, 6)
...
class Player(BasePlayer):
    ...
    def action_name(self):
        # example logic — adapt to your app
        if self.role() == "buyer":
            return "bid"
        return "ask"
            ...
            for p in self.group.get_players():
                if p.role() == target_role:
                    ...
                    if p.group.sequence_number <= 1:
                        ...
                        p.drop_sequence = 2
                        for i in range(p.round_number):
                            if p.in_round(i + 1).group.sequence_number in [1]:
                        ...
                        p.drop_sequence = 3
                        for i in range(p.round_number):
                            if p.in_round(i + 1).group.sequence_number in [1, 2]:
                                p.in_round(i + 1).payoff = p.in_round(i + 1).thisround_euro
                            else:
                                p.in_round(i + 1).payoff = 0
                    break
    ...
    def set_payoff(self):
        self.payoff = 0
        list_sequence = self.group.final_round_in_sequence()
        if self.disconnection_flag != 2:
            if self.round_number > list_sequence[self.subsession.paid_sequence_1 - 1] and self.round_number <= list_sequence[self.subsession.paid_sequence_1]:
                self.payoff = self.thisround_euro
            if self.round_number > list_sequence[self.subsession.paid_sequence_2 - 1] and self.round_number <= list_sequence[self.subsession.paid_sequence_2]:
                self.payoff = self.thisround_euro
    ...
    def get_repo_html(self, house):
        tpl = "double_auction/includes/repo_to_render.html" if house is False else "double_auction/includes/repo_to_render_btc.html"
    ...
    def get_asks_html(self, house):
        ...
        return mark_safe(render_to_string("double_auction/includes/asks_to_render.html", {"asks": asks, "player": self}))

    def get_bids_html(self, house):
        bids = self.group.get_bids(house)
        return mark_safe(render_to_string("double_auction/includes/bids_to_render.html", {"bids": bids, "player": self}))
    ...
    def get_contracts_html(self, house):
        tpl = "double_auction/includes/contracts_to_render.html" if house is False else "double_auction/includes/contracts_to_render_btc.html"
    ...
    def get_form_html(self, house):
        ...
        context["player"] = self
        tpl = "double_auction/includes/form_to_render.html" if house is False else "double_auction/includes/form_to_render_btc.html"
        return mark_safe(render_to_string(tpl, context))


    def profit_block_html(self, house):
        self.set_units(house)
        tpl = "double_auction/includes/profit_to_render.html" if house is False else "double_auction/includes/profit_to_render_btc.html"
        return mark_safe(render_to_string(tpl, {"player": self}))


    def general_info_block_html(self):
        return mark_safe(render_to_string(
            "double_auction/includes/general_info_to_render.html",
